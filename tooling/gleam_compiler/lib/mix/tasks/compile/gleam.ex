defmodule Mix.Tasks.Compile.Gleam do
  ...
  def compile_package(app, tests? \\ false) do
    ...
    if 0 < files do
      mix_lib = Path.join(Mix.Project.build_path(), "lib")
      build = Path.join(mix_lib, "#{app}")
      ...
      package =
        unless File.regular?("gleam.toml") do
          ...
          # Real Gleam project (engine): build deps natively so dependency
          # metadata (_gleam_artefacts) exists for import resolution.
          if @shell.cmd("gleam deps download && gleam build --target erlang") != 0 do
            raise MixGleam.Error, message: "gleam build failed"
          end
